...
class load_mySQL(object):
    ...
    def OK_interface(self):
        ...
        if input_year == "" or input_testname == "" or input_testnum_1 =="" or input_testnum_2 =="" or input_testobj =="" or input_testval =="":
            ...
            with conn.cursor() as cursor:
                val = "INSERT INTO `測試件結果_備機`(`年度次數`, `測試件項目編號`, `測試件分項目編號`, `年份`, `測試件序號`, `測試件結果`,`新增人員`) VALUES (%d, %d, %d, %s, %d, %.5f,'%s');" %(input_testnum_1,testname_num,testobj_num,input_year,input_testnum_2,input_testval,account)
                cursor.execute(val)
            conn.commit()
            conn.close()
    ...
    def gui_arrang(self):
        self.hellow_label.place(relx=0.05, rely=0.1, anchor=tk.W)
