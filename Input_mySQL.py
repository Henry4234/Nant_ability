        ...
        if Verify >= 1:
            ...
            with conn.cursor() as cursor:
                if nonequal == "=":
                    ...
                    val = "INSERT INTO `測試件結果`(`年度次數`, `測試件項目編號`, `測試件分項目編號`, `年份`, `測試件序號`, `測試件結果`,`不等判讀`,`新增人員`) VALUES (%d, %d, %d, %s, %d, %.5f,'%s','%s');" %(input_testnum_1,testname_num,testobj_num,input_year,input_testnum_2,input_testval,nonequal,account)
                cursor.execute(val)
            conn.commit()
