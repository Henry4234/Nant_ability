...
class output_mySQL(object):
    ...
    def OK_interface(self,x):   #x變數判定輸出主機或備機
        input_year = int(self.input_year.get())  #輸入年份擷取
        ...
        objnum = objnum[0]
        if input_year=="" or input_testnum=="" or input_testobj=="":
            ...
            with conn.cursor() as cursor:
                if x == 1:
                    srch_db="""SELECT `測試件結果`.`年份`,  `測試件結果`.`年度次數`, `測試件結果`.`測試件項目編號`, `測試件分項目`.`測試項目_分項`, `測試件結果`.`測試件序號`,`測試件結果`.`測試件結果`, `能力試驗結果`.`能力試驗數值`,`測試件結果`.`不等判讀` 
                            ...
                            AND `測試件結果`.`年份` = %s 
                            AND `測試件結果`.`年度次數` = %d
                            ORDER BY `測試件序號` ASC;"""%(input_testname,input_testobj,input_year,input_testnum)
                elif x != 1:
                    srch_db="""SELECT `測試件結果`.`年份`,  `測試件結果`.`年度次數`, `測試件結果`.`測試件項目編號`, `測試件分項目`.`測試項目_分項`, `測試件結果`.`測試件序號`,`測試件結果`.`測試件結果_備機`, `能力試驗結果`.`能力試驗數值`,`測試件結果`.`不等判讀_備機` 
                                ...
                                AND `測試件結果`.`年份` = %s 
                                AND `測試件結果`.`年度次數` = %d 
                                ORDER BY `測試件序號` ASC;65
                                """%(input_testname,input_testobj,input_year,input_testnum)
            ...
            try:
                ...
                srch_presetclsi = """SELECT `clsi規則`.`規則內容`,`clsi規則`.`實際數值`,`clsi規則`.`實際數值_1`
                                    ...
                                    WHERE `測試項目_分項` = '%s';"""%(input_testobj)
                with conn.cursor() as cursor:
                    cursor.execute(srch_presetclsi)
                prerules = cursor.fetchone()
                pre = prerules[0]
                if tk.messagebox.askyesno(title='南投醫院檢驗科',message='%s是否先使用預設可容許範圍:%s?'%(input_testobj,pre)):
                    aa = pre
                    rules = list(prerules)
                    rules.insert(0,"")
                    rules.insert(0,"")
            ...
            wb = Workbook()
            ws = wb.active
            ws.title = "%s_%d_%s_%s"%(input_year,input_testnum,self.input_testname.get(),self.input_testobj.get())
            # ws.title()
            title=["年份","年度次數","測試件項目","測試件分項目","測試件序號","測試件結果","目標值"]
            ws.append(title)
            #放入目標數值
            if cnt!= objnum:    #如果database.rowcount與預設測試件件數不符，帶入缺少函數中
                name = self.db_insufficient(result_db = name, objnum = objnum)
                print(name)
            else: 
                name = pd.DataFrame(name)   #如果上述件數相符，直接將rawdata轉換為dataframe格式
            for i in range(0, len(name)):   #逐行放入excel且判讀不等資訊
                row_name = name.iloc[i]
            ...
            for i in range(2,len(name)+2):
                if ws['B' + str(i)].value == None:
                    ...
                    actualmean = ws['B' + str(i)].value
                    peermean =  ws['C' + str(i)].value
                    try:
                        ws['D' + str(i)].value = actualmean - peermean
                    except TypeError:
                        continue
            ##新增標題
            ws.insert_rows(1)
            ws['A1'] = "%s年第%d次%s_%s"%(input_year,input_testnum,self.input_testname.get(),self.input_testobj.get())
            ...
            if "or" in aa:  #如果多於兩個變數
                if "%" and "SD" in aa:  #如果同時出現SD跟%
                    ...
                    if sd_row != len(name):
                        sd = self.sd_insufficient(testnamenum=input_testname,testobj=input_testobj,year=input_year,testnum=input_testnum,objnum=objnum)
                    else:
                        sd = pd.DataFrame(sd)
                    sd = sd.values.tolist()
                    for i in range(3,len(name) + 3):
                        if ws['B' + str(i)].value == None:
                            ...
                            realamount_1 = rules[3] #百分比
                            realamount_2 = rules[4] #SD
                            realamount_1 = ws['C' + str(i)].value * realamount_1
                            realamount_2 = sd[i-3][2] * realamount_1
                            if realamount_1 > realamount_2: #如果百分比的比較大
            ...
            ydata = Reference(ws, min_col=4, min_row=4, max_col=6, max_row=len(name)+4)
            xvalue = Reference(ws, min_col=1, min_row=5, max_col=1, max_row=len(name)+5)
            ...
            if ws['E3'].value =="計算複雜，請自行計算":
                ws['C1'].value = aa
                tk.messagebox.showinfo(title='南投署立醫院檢驗科', message='可容許範圍判定大於兩個變數，請自行判斷')
            else:
                if x == 1:
                        ...
                        with conn.cursor() as cursor:
                            srch_num ="""SELECT `測試件結果`.`結果編號` 
                                        ...
                                        WHERE `測試件結果`.`測試件分項目編號` IN  (
                                            ...
                                            AND `測試項目_分項` = '%s')
                                        AND `測試件結果`.`年份` = %s 
                                        AND `測試件結果`.`年度次數` = %d ;"""%(input_testname,input_testobj,input_year,input_testnum)
                            cursor.execute(srch_num)
                        rowcnt = cursor.rowcount 
                        if rowcnt != objnum:
                            srch_number = self.update_insufficient(testname=input_testname,testobj=input_testobj,year=input_year,testnum=input_testnum,objnum=objnum)
                        # print(srch_number)
                        else:
            ...
            with conn.cursor() as cursor:   #取得近五年
                ...
                LIMIT %d;"""%(input_testname,input_testobj,objnum*5)
            ...
            if anscount % objnum !=0:
                msg = "%s年%s項目不足!"%(int(year[i][0]),input_testobj)
                tk.messagebox.showerror(title='南投署立醫院檢驗科', message = msg)
                ans = self.backup_interface(haveyear=year,objnum=objnum)
                ans.reset_index()
                print(ans)
            else:
            ...
            ##歷次監控文字格式設定
            ws['A25'].font = ft2
            pastfont = "A26:%s26"%(get_column_letter(len(title)))
            ...
            ydata = Reference(ws, min_col=1, min_row=27, max_col=6, max_row=len(name)+27)
    ...
    def db_insufficient(self,result_db,objnum): #database缺少函數:如果能力試驗有缺件(1245)，自動空排
    ...
    def sd_insufficient(self,testnamenum,testobj,year,testnum,objnum): #解決如果能力試驗不足五項，但需要利用SD計算可容許範圍時
        ...
        return ans
    def backup_interface(self,haveyear,objnum): #歷年不足五次救援
        input_testobj = str(self.input_testobj.get()) #輸入分項目擷取str
        input_testname = testname_dict[self.input_testname.get()]
        input_testname = int(input_testname)
        # print(haveyear)
        ...
        for i in range(0,len(haveyear)):
            with conn.cursor() as cursor:
                get_past = """SELECT `能力試驗結果`.`測試件結果編號`, `測試件結果`.`年份`,`測試件結果`.`年度次數`,`測試件結果`.`測試件序號`,`能力試驗結果`.`差異百分比`
                                ...
                                AND `測試件結果`.`年度次數`=%d
                                ORDER BY `測試件結果`.`年份` DESC, `測試件結果`.`年度次數` DESC, `測試件結果`.`測試件序號` ASC
                                LIMIT %d;"""%(input_testname,input_testobj,int(haveyear[i][0]),int(haveyear[i][1]),objnum)
                cursor.execute(get_past)
            ans = cursor.fetchall()
    ...
    def gui_arrang(self):
        ...
        self.label_testobj.place(relx=0.23,rely=0.5,anchor=tk.W)
        ...
        self.input_testobj.place(relx=0.45,rely=0.5,anchor=tk.W)
